...
export default {
    ...
    methods: {
        ...
        actionSave() {
            ...
            this.$Modal.confirm({
                title: '预收款添加确认',
                content: '是否已经确认收到客户:' + self.addFormItem.custName + '金额' + self.addFormItem.logAmount + '元',
                onCancel: () => {},
                onOk: () => {
        ...
        cancelBtnClick() {
            ...
            this.$Modal.confirm({
                title: '取消操作确认',
                content: '是否确认取消:' + self.currChooseRow.bizNo + "这笔预收款，取消后不能再做其他操作！",
                onCancel: () => {},
                onOk: () => {
                    self.loading = true;
        ...
        offsetBtnClick() {
            ...
            //先根据custId获取供应商信息
            let self = this;
            util.ajax.get('/customer/' + this.currChooseRow.custId)
                .then((response) => {
        ...
        offsetSubmit() {
            if (this.offsetFormItem.bizType !== 'PRE_RECEIVE' || !this.offsetFormItem.preRecordId) {
                this.$Modal.error({
                    title: '系统异常',
            ...
            this.$Modal.confirm({
                title: '冲销数据确认',
                content: '是否确认对客户：' + self.offsetFormItem.custName + '冲销' + self.offsetFormItem.logAmount + '元预收款, 冲销后不可取消',
                onCancel: () => {},
                onOk: () => {
