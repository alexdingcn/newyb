...
export default {
    ...
    methods: {
            ...
            if (this.supplierId) {
                ...
                util.ajax.get('/supplier/' + this.supplierId)
                    .then(function (response) {
                        ...
                        self.businessScopeChooseList = self.formItem.businessScopeIdList ? self.formItem.businessScopeIdList : [];
                    })
        ...
        submitSupplier (action) {
            var self = this;
            self.supplierLoading = true;
            util.ajax.post('/supplier/save', this.formItem)
                .then(function (response) {
                    self.supplierLoading = false;
                    self.formItem = response.data;
        ...
        saveContact () {
        	...
        	if (this.formItem.id) {
        		...
        		util.ajax.post('/supplier/contact/save', this.contact)
        			.then(function (response) {
                        self.$Message.success('供应商业务代表' + self.contact.name + '保存成功');
        ...
        removeContact(row) {
            ...
            this.$Modal.confirm({
                ...
                onOk: () => {
                    util.ajax.post('/supplier/contact/remove/' + row.id)
                        .then(function (response) {
