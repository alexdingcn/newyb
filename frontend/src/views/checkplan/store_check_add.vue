...
<template>
    <div>
        <Card>
            <p slot="title" >
                <FontIcon type="icon-cangkucangchu"></FontIcon>盘点制单 
            </p>
            <div slot="extra">
                <ButtonGroup >
                    <Button type="primary" icon="plus"  @click="CheckPlanGetBtnClick">载入盘点计划</Button>
                    <Button type="success" icon="checkmark" @click="addCheckOrder">保存盘点计划</Button>
                </ButtonGroup>
            </div>
            ...
            <Form :label-width="85" :rules="ruleValidate" :model="storeCheck" ref="CheckOrderForm">
                <Row class="row-margin-bottom">
                    ...
                    <FormItem label="仓库点" prop="warehouseId">
                        <warehouse-select v-model="storeCheck.warehouseId"  :disabled="disableWarehouse"></warehouse-select>
                    </FormItem>
                    </i-col>
                    <i-col span="5">
                    ...
                    <FormItem label="盘点日期" prop="checkDate" :disabled="disable_checkdate">
                        <DatePicker type="date" v-model="storeCheck.checkDate" />
                    </FormItem>
                    </i-col>
                </Row>
                <Row class="row-margin-bottom">
                    ...
                    <i-col span="5">
                        <FormItem label="备注" prop="comment">
                            <Input v v-model="storeCheck.comment" :disabled="disable_note"/>
                        </FormItem>
                    </i-col>
  ...
  methods: {
    ...
    queryGoods(query) {
      ...
      if (query !== "") {
        ...
        util.ajax
          .get("/goods/list", {
            params: { search: query, page: 1, size: 10 }
          })
    ...
    doSave() {
      ...
      this.saving = true;
      util.ajax
        .post("/repertory/check/add", this.storeCheck)
        .then(function(response) {
    ...
    refreshCheck(checkPlanId) {
      ...
      util.ajax
        .post("/repertory/check/orderinfoList?checkPlanId=" + checkPlanId)
        .then(function(response) {
