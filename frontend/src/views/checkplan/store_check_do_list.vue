                ...
                <Form :label-width="85" :rules="ruleValidate" :model="checkFormInfo" ref="CheckPartDetailForm">
                    <Row>
                        ...
                        <FormItem label="盘点表编号" prop="formNo">
                            <Input v-model="checkFormInfo.formNo" size="small" :disabled="true"/>
                        </FormItem>
                        </i-col>
                        <i-col span="6">
                        <FormItem label="备注" prop="checkNote">
                            <Input v-model="checkFormInfo.checkNote" size="small"   />
                        </FormItem>
                        </i-col>
                        <i-col span="10">
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            if (self.orderItems[params.index]["formStatus"] == "CHECKED") {
              return h(
                "span",
                self.orderItems[params.index][params.column.key]
              );
            } else {
              return h("Input", {
                props: {
                    ...
                    if (checkLimit > row.accLimit) {
                      ...
                      self.$Modal.confirm({
                        ...
                        onOk: () => {
                          util.ajax
                            .post(
                              "/repertory/check/doCheckDetail",
                              self.checkMore
                            )
                            .then(function(response) {
                              if (response.status === 200 && response.data) {
                                row.checkStatus =
                                  response.data.planDetail.checkStatus;
                      ...
                      util.ajax
                        .post("/repertory/check/doCheckDetail", self.checkMore)
                        .then(function(response) {
                          if (response.status === 200 && response.data) {
                            row.checkStatus =
                              response.data.planDetail.checkStatus;
  ...
  methods: {
    ...
    initPart(checkPlanId, warehouseName) {
      ...
      if (checkPlanId > 0) {
        util.ajax
          .post(
            "/repertory/check/orderinfoListByUser?checkPlanId=" + checkPlanId
          )
          .then(function(response) {
    ...
    backPartIndex() {
      this.$router.push({
        ...
        params: {
          ...
          warehouseName: this.storeCheck.warehouseName
        }
      });
    },
    queryRepertory(query) {
      ...
      if (query !== "") {
        ...
        util.ajax
          .post(
            "/repertory/check/orderinfoList4Search?checkPlanId=" +
              this.checkPlanId +
              "&goodSearch=" +
              query
          )
    ...
    saveCheckPart() {
      ...
      if ("" != this.checkFormInfo.id && this.checkFormInfo.id > 0) {
        this.saving = true;
        util.ajax
          .post("/repertory/check/addOrUpdateForm", this.checkFormInfo)
          .then(function(response) {
            if (response.status === 200 && response.data) {
    ...
    handleAddLocation() {
      var self = this;
      util.ajax
        .post("/repertory/check/doCheckDetailMore", this.checkMore)
        .then(function(response) {
          if (response.status === 200 && response.data) {
