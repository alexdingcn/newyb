                    ...
                    <div>
                        <Form ref="userForm" :model="userForm" :rules="userFormRule" :label-width="100">
                            <FormItem label="用户名：" prop="nickname" :error="nicknameError" >
                                <Input v-model="userForm.nickname" :maxlength="30" :disabled="editView" @on-blur="validateNickNameExist" ></Input>
                            </FormItem>
                            <FormItem label="用户手机：" prop="mobile" :error="mobileError" >
                                <Input v-model="userForm.mobile" :maxlength="11" :disabled="editView" @on-blur="validMobileExist"></Input>
                            </FormItem>
                            <FormItem label="真实姓名：" prop="realname">
                                <Input v-model="userForm.realname" ></Input>
                            </FormItem>
                            <FormItem label="初始登录密码：" :error="passwordError">
                                <Input type="password" v-model="userForm.password" :disabled="editView"></Input>
                            </FormItem>
                            <FormItem label="电子邮箱：" >
...
export default {
    ...
    methods: {
        ...
        validateNickNameExist() {
            ...
            util.ajax.get('/user/valid/nickname', {params: {nickname: nickname}})
                .then((response) => {
                    this.nicknameError = '';
                })
                .catch((error) => {
        ...
        doSave() {
            ...
            util.ajax.put('/user/save', this.userForm)
                .then((response) => {
                    this.$Message.success('保存成功, 如果是新建的用户，需要在权限管理页面进行激活并分配权限.');
