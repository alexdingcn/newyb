...
<template>
   <Select v-model="id" filterable clearable :disabled="disabled" placeholder="请选择支付方式方式" @on-change="onChange">
...
export default {
    ...
    methods: {
        initList() {
            let reqData = ["PAY_METHOD"];
            util.ajax
                .post("/options/list", reqData)
                .then(response => {
                    this.optionList = response.data.PAY_METHOD;
                })
                .catch(error => {
