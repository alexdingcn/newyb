...
<template>
   <Select v-model="id" :size="size" filterable clearable :disabled="disabled" placeholder="请选择采购属性" @on-change="onChange">
...
export default {
    ...
    methods: {
        initList() {
            let reqData = ["BUY_TYPE"];
            util.ajax
                .post("/options/list", reqData)
                .then(response => {
                    this.optionList = response.data.BUY_TYPE;
                })
                .catch(error => {
