...
<template>
   <Select v-model="id" :size="size" filterable clearable :disabled="disabled" placeholder="请选择出库类型" @on-change="onChange">
...
export default {
    ...
    methods: {
        initList() {
            let reqData = ["WAREHOUSE_OUT_TYPE"];
            util.ajax
                .post("/options/list", reqData)
                .then(response => {
                    this.optionList = response.data.WAREHOUSE_OUT_TYPE ;
                })
                .catch(error => {
