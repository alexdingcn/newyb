...
export default {
            ...
            orderListColumns: [
                ...
                },
                {
                    ...
                    render: (h, params) => {
                        ...
                        let label = params.row.status === 'CHECKED' ? '已审查' : '未审查';
                        return  h('Tag', {
                            props: {
                                color: color
                            }
...

                {
                    ...
                    render: (h, params) => {
                        ...
                        return outDate ? moment(outDate).format("YYYY-MM-DD") : '';
                    }
                },
                {
                    title: '备注',
    ...
    watch: {
        detailList(data) {
            if(!data || data.length <= 0) {
                ...
                this.totalOutCount = data.reduce((total, item) => {return item.quantity ? total + item.quantity : total + 0}, 0);
    ...
    methods: {
        ...
        reloadOrderDetail() {
            ...
            util.ajax.get('/repertory/out/detail/' + this.currentChooseOrder.id)
                .then((response) => {
        ...
        checkOk() {
            ...
            this.$Modal.confirm({
                ...
                onOk: () => {
                    util.ajax.put('/repertory/out/set/outcheck', reqData)
                        .then((response) => {
